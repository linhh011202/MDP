...
void PID_init_oldmethod(pid_type_def *pid, uint8_t mode, const float PID[3], float max_out, float max_iout, float max_dout, float dT)
{
    if (pid == NULL || PID == NULL)
    {
        return;
...
float PID_calc_oldmethod(pid_type_def *pid, float input, float target) {
	  if (pid == NULL)
    {
        return 0.0f;
    }
