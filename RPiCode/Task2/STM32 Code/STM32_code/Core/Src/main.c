...
const osThreadAttr_t sensorTask_attr = { .name = "sensorTask", .stack_size =
		4096, .priority = (osPriority_t) osPriorityNormal, };
...
int main(void)
{
  ...
  HAL_GPIO_WritePin(Collision_Ind_Port, Collision_Ind_Pin, GPIO_PIN_RESET);
	...
	/**! FROM THIS POINT THE LOGIC CONTINUES IN APP_MAIN.CPP
	 *
	 * It is recommended to not use main.c..
	 * (1) The ioc code generation will overwrite parts if not careful to place them within USER CODE sections
	 * (2) The ioc code generation will not generate for c++ main file.
	 * **/
