...
public class BluetoothPopUp extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        oDeviceList = findViewById(R.id.oDeviceList);
        pDeviceList = findViewById(R.id.pDeviceList);
        ...
        oDeviceList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @SuppressLint("MissingPermission")
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                mBluetoothAdapter.cancelDiscovery();
                pDeviceList.setAdapter(mPairedDevlceListAdapter);
        ...
        pDeviceList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @SuppressLint("MissingPermission")
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                mBluetoothAdapter.cancelDiscovery();
                oDeviceList.setAdapter(mNewDevlceListAdapter);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothDevice.ACTION_FOUND)) {
                ...
                if (device.getName() != null && !device.getName().isEmpty()) {
                    ...
                    for (BluetoothDevice listedDevice : mNewBTDevices) {
                        // Use Objects.equals() for null-safe comparison
                        if (Objects.equals(listedDevice.getName(), device.getName())) {
                            isAlreadyAdded = true;
                            break;
                        }
                    }

                    // Add the device if it's not already added
                    if (!isAlreadyAdded) {
                        mNewBTDevices.add(device);
                        Log.d(TAG, "onReceive: Added " + device.getName());
                    }

                    // Update or initialize the device list adapter
                    if (mNewDevlceListAdapter == null) {
                        mNewDevlceListAdapter = new DeviceListAdapter(context, R.layout.device_adapter_view, mNewBTDevices);
                        oDeviceList.setAdapter(mNewDevlceListAdapter);
                    } else {
                        mNewDevlceListAdapter.notifyDataSetChanged(); // Notify the adapter of the data change
                    }
                }
            }
