...
class RaspberryPi:
    ...
    def start(self):
        ...
        try:
            ...
            image_id = 'NA'
            while image_id == 'NA':
                image_id = self.snap_and_rec("1_1")
                time.sleep(1)
    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        """
        RPi snaps an image and calls the API for image-rec using picamera.
        ...
        # Basic capture with no camera settings
        with picamera.PiCamera() as camera:
            camera.capture(filename)

        if not os.path.exists(filename):
            print(f"Image file {filename} was not created!")
            return
