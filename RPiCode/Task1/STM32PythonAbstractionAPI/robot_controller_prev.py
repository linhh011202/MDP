...
# In your RobotController class
    ...
    def is_busy(self) -> Optional[bool]:
        """Requests the robot's busy status from the firmware."""
        self.drv.construct_cmd()
        self.drv.add_cmd_byte(False) # This is a request ('q')
        self.drv.add_module_byte(self.drv.Modules.SENSOR)
        
        # Use the new Enum member instead of the raw bytes b'b'
        self.drv.add_sensor_byte(self.drv.SensorCmd.IS_BUSY) 
        
        self.drv.pad_to_end()
        ret = self.drv.send_cmd()

        # Check the response from the C code
        if '1' in ret:
            return True
        if '0' in ret:
            return False
        return None # Return None if the response was not '0' or '1'

    def wait_until_idle(self, timeout_s: float = 10.0):
        ...
        while time.time() - start_time < timeout_s:
            if not self.is_busy():
                print("Robot is idle.")
                return True
            time.sleep(0.5) # Poll 10 times per second
