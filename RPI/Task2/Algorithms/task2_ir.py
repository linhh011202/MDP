...
from stm32_api import robot_controller

robot = robot_controller.RobotController(PORT, BAUD)
...
# === APPEND BELOW (end of file) =============================================

def _decode_left_right(image_id):
    """
    Try to resolve the server's image_id to a human label.
    - First try consts.SYMBOL_MAP (id -> label) if available.
    - Then accept direct string labels ("left"/"right") if the server returns text.
    - Return 'left'/'right' or None.
    """
    try:
        # Optional: only if you have consts.SYMBOL_MAP available like in your second script
        from consts import SYMBOL_MAP  # maps id (str) -> label (e.g. 'Left')
        lbl = SYMBOL_MAP.get(str(image_id))
    ...
    # Fallback if the server already returns the label directly
    s = str(image_id).strip().lower()
    if s in ("left", "right"):
        return s

    # Unknown / not left-right
    return None
...
def capture_and_classify_left_right():
    ...
    try:
        ...
        _API_IP = os.environ.get("API_IP", "127.0.0.1")
...
if __name__ == '__main__':
    ...
    import time
    start = time.time()
    capture_and_classify_left_right()
    print(time.time() - start)
    try:
        while True:
            print("Robot ready. Press start on android.")
            if "start" == rpi.android_receive():
                # Take an image, send to server, and print LEFT/RIGHT if recognized.
                capture_and_classify_left_right()
                break
    finally:
